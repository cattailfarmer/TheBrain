...
`coordination/run_local_to_merge_packet_bridge_design.sop` defines the next bridge. Eligible run-local outputs may become draft accepted-file-map inputs for a later manual merge packet, but the bridge output is still not a `ManualMergePacket` and still cannot apply files.
